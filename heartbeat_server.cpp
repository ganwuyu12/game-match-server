...
void send_match_result(int fd, bool success, int room_id = 0, const string& color = "") {
    json resp;
    if (success) {
...
void try_match() {
    while (g_match_queue.size() >= 2) {
        ...
        if (abs(a.score - b.score) <= SCORE_RANGE) {
            ...
            send_match_result(a.fd, true, room_id, "black");
            send_match_result(b.fd, true, room_id, "white");
        } else {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (fd == lfd) {
                ...
                if (ret <= 0) {
                    ...
                    for (auto& kv : g_rooms) {
                        ...
                        if (room.player1_fd == fd) {
                            room.game_over = true;
                            room.winner = 2;
                            send_json(room.player2_fd, {{"cmd", "opponent_quit"}});
                        } else if (room.player2_fd == fd) {
                            room.game_over = true;
                            room.winner = 1;
                            send_json(room.player1_fd, {{"cmd", "opponent_quit"}});
                        }
                    }
                    continue;
                }
                ...
                while (!g_match_queue.empty()) {
                    Player p = g_match_queue.front();
                    g_match_queue.pop();
                    if (p.fd == fd) p.last_heartbeat = time(nullptr);
                    tmp.push(p);
                }
                g_match_queue = tmp;
                ...
                try {
                    ...
                    if (cmd == "match") {
                        ...
                        cout << "玩家 " << fd << " 加入匹配队列，等待: " << g_match_queue.size() << endl;
                        try_match();
